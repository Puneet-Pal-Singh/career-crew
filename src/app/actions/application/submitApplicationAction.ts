...
export async function submitApplication(
  ...
  try {
    ...
    if (jobData.employer_id === user.id) {
      console.warn(`Server Action (${actionName}): Employer ${user.id} attempting to apply to their own job ${jobId}.`);
    ...
    if (checkError) {
      console.error(`Server Action (${actionName}): Error checking for existing application for job ${jobId}, user ${user.id}.`, checkError);
      return { success: false, error: "Could not verify previous applications. Please try again." };
    }
    if (existingApplication) {
      console.log(`Server Action (${actionName}): User ${user.id} has already applied to job ${jobId}.`);
    ...
    console.log(`Server Action (${actionName}): Inserting application for job ${jobId}, user ${user.id}.`);
    ...
    if (insertError) {
      console.error(`Server Action (${actionName}): Supabase insert error for application (job ${jobId}, user ${user.id}).`, insertError);
    ...
    console.log(`Server Action (${actionName}): Application submitted successfully. App ID: ${newApplication.id} for job ${jobId}, user ${user.id}.`);
