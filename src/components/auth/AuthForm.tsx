...
// // FIX 1: Correctly define the discriminated union for the props.
// // The 'login' mode has no role, the 'register' mode requires one.
// type AuthFormProps = {
//   mode: 'login';
// } | {
//   mode: 'register';
...
//     // FIX 2: Use the type guard 'if (props.mode === 'register')'
//     // This tells TypeScript that inside this block, 'props.role' is guaranteed to exist.
...
//         return;
//       }
//       // We pass the role from props here
//       result = await registerUserAction({ ...values, fullName: values.fullName, role: props.role });
//     } else {
//       // The login action is simpler and does not need a role.
...
//     <Card className="w-full">
//       <CardHeader className="text-center">
//         <CardTitle className="text-2xl">{isRegisterMode ? 'Create Account' : 'Log In'}</CardTitle>
//         <CardDescription>
//           {isRegisterMode ? (props.role === 'JOB_SEEKER' ? 'Find your next opportunity.' : 'Find the best startups and talent.') : 'Welcome back! Log in to continue.'}
//         </CardDescription>
//       </CardHeader>
//       <form onSubmit={form.handleSubmit(onSubmit)}>
//         <CardContent className="space-y-4">
...
//           )}
//           <div className="space-y-2">
//             {/* Show "Work Email" for employers, just "Email" for others */}
//             <Label htmlFor="email">{isRegisterMode && props.role === 'EMPLOYER' ? 'Work Email' : 'Email'}</Label>
...
//           <p className="text-sm text-muted-foreground text-center">
//             {isRegisterMode ? 'Already have an account?' : "Don't have an account?"}
//             {/* FIX: Login page ("!isRegisterMode") now correctly links to the seeker signup page */}
//             <Link 
//               href={isRegisterMode ? '/login' : '/signup/job-seeker'} 
//               className="ml-1 font-semibold text-primary hover:underline"
//             >
